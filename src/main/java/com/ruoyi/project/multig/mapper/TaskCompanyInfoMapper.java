...
public interface TaskCompanyInfoMapper {
    ...
    @Select("SELECT id, task_info, company_info, date_number, date_time FROM evaluate_info WHERE task_info= #{taskId} AND company_info = #{companyId}")
    @Results({
    ...
    TaskCompanyInfo selectByTaskIdAndCompanyId(@Param("taskId") Long taskId, @Param("companyId") Long companyId);
}
