...
public class GA {
    ...
    private double p1;
    private double p2;
    private Map<String, List<Integer>> Tmachine;
    private Map<String, List<Integer>> Tmachine1;
    private Map<String, List<Double>> Tmachinetime;
    private Map<String, List<Double>> Tmachinecost;
    private Map<String, List<Integer>> tdx;
    private Map<String, List<Integer>> tdx_cost;
    private List<Integer> work;
    private Map<String, List<Integer>> tom;
    private Map<String, List<Integer>> tom_cost;
    ...
    public GA(int generation, int popsize, double p1, double p2, List<Object> parmData,
        ...
        this.p1 = p1;
        this.p2 = p2;
        this.Tmachine = (Map<String, List<Integer>>) parmData.get(0);
        this.Tmachine1 = (Map<String, List<Integer>>) parmData.get(1);
        this.Tmachinetime = (Map<String, List<Double>>) parmData.get(2);
        this.Tmachinecost = (Map<String, List<Double>>) parmData.get(3);
        this.tdx = (Map<String, List<Integer>>) parmData.get(4);
        this.tdx_cost = (Map<String, List<Integer>>) parmData.get(5);
        this.work = (List<Integer>) parmData.get(6);
        this.tom = (Map<String, List<Integer>>) parmData.get(7);
        this.tom_cost = (Map<String, List<Integer>>) parmData.get(8);
        this.machines = (Map<String, Integer>) parmData.get(9);
        this.businessToGroupMap=(Map<String, String >) parmData.get(10);
        this.businessToLinkMap=(Map<String, String >) parmData.get(11);
        this.machineKeys = new ArrayList<>(machines.keySet()); // 保存 machines 映射的键
   ...
   /* public int[][] globalInitial() {
        int[][] MS = createZeroMatrix(GS_num, work.size());
        int[][] OS = createZeroMatrix(GS_num, work.size());
        ...
        for (int i = 0; i < GS_num; i++) {
            ...
            int operationIndex = 0; // 初始化工序位置索引
            for (String g : machines.keySet()) { // 循环遍历任务ID
                int subtaskCount = machines.get(g); // 获取任务的子任务数
                for (int j = 0; j < subtaskCount; j++) {
                    // ... 现有的选择机器和负荷计算逻辑 ...
                    int highs = tdx.get(g); // 可能需要根据实际情况调整
                    int lows = 0; // 根据需要调整
                    ...
                    for (int k = lows; k < highs; k++) {
                        ...
                        corrTime.add(Tmachinetime.get(g).get(k));
                    }
                    // 目标函数是负荷+处理时间最小
                    int minLoadIndex = 0;
                    double minLoadValue = Double.MAX_VALUE;
                    for (int k = 0; k < machineFeasible.size(); k++) {
                        double loadValue = listLoad.get(machineFeasible.get(k)) + corrTime.get(k);
                    ...
                    // 更新MS数组的对应工序位置
                    MS[i][operationIndex] = selectedMachine;
                    operationIndex++; // 更新工序位置索引
                }
            }
        }
        // 合并 MS 和 OS
    ...
    public int[][] globalInitial() {
        ...
        List<String> GJ_list = new ArrayList<>(machines.keySet()); // 创建机器列表副本
    ...
    // 实现 sum 方法
    private int sum(Map<String, Integer> machines, String upToKey) {
        int sum = 0;
    ...
    /*public int[][] localInitial() {
        ...
        for (int i = 0; i < LS_num; i++) {
            ...
            List<Integer> listLoad = new ArrayList<>(Collections.nCopies(machineNum, 0)); // 初始化机器负荷为0
            int operationIndex = 0; // 添加一个计数器来跟踪工序的累加位置
            for (String g : machines.keySet()) { // 循环遍历任务ID
                int subtaskCount = machines.get(g); // 获取任务的子任务数
                for (int j = 0; j < subtaskCount; j++) {
                    int highs = tdx.get(g); // 可能需要根据实际情况调整
    ...
    /*public int[][] randomInitial() {
        ...
        for (int i = 0; i < RS_num; i++) {
            ...
            List<Integer> GJ_list = new ArrayList<>(jobList); // 使用jobList的副本
            Random rand = new Random();
            for (Integer g : GJ_list) {
                for (int j = 0; j < machines.get(g); j++) {
                    int index = work.subList(0, g).stream().mapToInt(Integer::intValue).sum() + j;
                    if (index < MS[i].length) {
                        MS[i][index] = rand.nextInt(tdx.get(g));
                    } else {
    ...
    public int[][] randomInitial() {
        ...
        for (int i = 0; i < RS_num; i++) {
            ...
            Random rand = new Random();
            List<String> GJ_list = new ArrayList<>(machines.keySet()); // 使用machines的键集副本
            for (String g : GJ_list) {
                int subtaskCount = machines.get(g); // 获取任务的子任务数
    ...
    public DecodeResult decode(int[] chrom) {
       ...
       // System.out.println("Machines map: " + machines);
        ...
        for (int i : OS) {
            ...
            count[i]++;
            String upToKey = Integer.toString(i+1); // 将 i 转换为字符串，以匹配 machines 中的键格式
            int index = sum(machines, upToKey) + count[i];
...
// 打印 mListCost 集合中每个元素的类型
    ...
    public MutationResult mutationMachine(int[] job, int[] machine, double[] machineTime, double[] machineCost) {
        ...
        for (int i : T_r) {
            ...
            /*System.out.println("nMachine"+nMachine);
    ...
    public CalculationResult calculate(int[] job, int[] machine, double[] machineTime, double[] machineCost) {
        ...
        for (int i = 0; i < job.length; i++) {
            //System.out.println("Iteration: " + i + ", Machine value: " + machine[i] + ", tmm length: " + tmm.length);
    ...
    public Object[] gaInitial() {
        ...
        double[][] WorkC2 = null;
        /*// 打印 Tmachine
        System.out.println("Tmachine: " + Tmachine);

// 打印 Tmachine1
        System.out.println("Tmachine1: " + Tmachine1);

// 打印 Tmachinetime
        System.out.println("Tmachinetime: " + Tmachinetime);

// 打印 Tmachinecost
        System.out.println("Tmachinecost: " + Tmachinecost);

// 打印 tdx
        System.out.println("tdx: " + tdx);

// 打印 tdx_cost
        System.out.println("tdx_cost: " + tdx_cost);
...
// 打印 tom_cost
        System.out.println("tom_cost: " + tom_cost);

// 打印 machines
        System.out.println("machines: " + machines);*/

        // 以下为全局、局部、随机初始化
        //int[][] globalInit = globalInitial(); // globalInitial 返回 int[][]
        /*or (int i = 0; i < GS_num; i++) {
            CHS[i] = globalInit[i];
        }
        int[][] localInit = localInitial(); // localInitial 返回 int[][]
        for (int i = 0; i < LS_num; i++) {
            CHS[i + GS_num] = localInit[i];
        }*/
        // 随机初始化
        ...
        // 打乱 CHS 数组
        shuffleCHS(CHS);
        //System.out.println(workM[0].length);
        // 解码和计算适应度
        for (int i = 0; i < popsize; i++) {
            ...
            /*System.out.println("workJob: " + Arrays.toString(workJob[i]));
            System.out.println("workM: " + Arrays.toString(workM[i]));
            System.out.println("workT: " + Arrays.toString(workT[i]));
            System.out.println("workC: " + Arrays.toString(workC[i]));*/
            //System.out.println("i="+i+","+workM[i].length);
    ...
    public Object[]  gaTotal(Map<Double, Object> newCar, String initialTime) {
        System.out.println("gaTotal method called with initialTime: " + initialTime);
        Map<Pair, Double> errorMT = new HashMap<>();
        Map<Pair, Double> modifiedErrorMT = new HashMap<>();
        List<Object> dataList = new ArrayList<>();
        int[] factoryInfo = {1, 1, 1, 2, 2, 2};
        int[] linkInfo = {1, 2, 3, 1, 2, 3};
        List<Double> arriveTime = new ArrayList<>(newCar.keySet());

        // 如果没有新到的车辆
        if (arriveTime.isEmpty()) {
            // 执行没有新车辆的逻辑，可能是直接返回当前的调度结果
            System.out.println("没有新任务进入");
...
// 计算未完成的工件及其剩余工序数
            ...
            Map<String, List<Integer>> TmachineCopy = deepCopyMapOfList(Tmachine);
            Map<String, List<Integer>> Tmachine1Copy = deepCopyMapOfList(Tmachine1);
            Map<String, List<Double>> TmachinetimeCopy = deepCopyMapOfListDouble(Tmachinetime);
            Map<String, List<Double>> TmachinecostCopy = deepCopyMapOfListDouble(Tmachinecost);
            Map<String, List<Integer>>tdxCopy = new HashMap<>(tdx);
            Map<String, List<Integer>> tdx_costCopy = new HashMap<>(tdx_cost);
            //List<Integer> workCopy = new ArrayList<>(work);
            Map<String, List<Integer>> tomCopy = new HashMap<>(tom);
            Map<String, List<Integer>> tom_costCopy = new HashMap<>(tom_cost);
            Map<String, Integer> machinesCopy = new HashMap<>(machines);
            // 遍历 counter 来更新 work、machines 和 tdx
            for (Map.Entry<Integer, Integer> entry : counter.entrySet()) {
                String key = entry.getKey().toString(); // 工件
            ...
            String jobKey = String.valueOf(jobNum);

            jobList.add(jobNum);
            int numCar = (Integer) car.get(0); // 假设车辆信息的第一个元素是数量
            for (int i = 0; i < numCar; i++) {
                work.add(jobNum); // 假设 work 是一个整型列表
            }
            jobNum++;
            ...
            for (Map.Entry<Pair, Double> entry : modifiedErrorMT.entrySet()) {
        ...
        return new Object[] {errorMT, dataList};
        }
    private <T> Map<String, List<T>> deepCopyMapOfList(Map<String, List<T>> original) {
    ...
    private Map<String, List<Double>> deepCopyMapOfListDouble(Map<String, List<Double>> original) {
